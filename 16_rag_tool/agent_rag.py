...
def query_video_transcript(query: str) -> str:
    ...
    print(f"\n[Tool Execution: query_video_transcript] Searching database for query: '{query}'...")
    if retriever is None:
        return "Error: FAISS retriever is not initialized."
    try:
        results = retriever.invoke(query)
        context = "\n\n".join(doc.page_content for doc in results)
        print(f"[Tool Execution: query_video_transcript] Retrieved {len(results)} chunks.")
        return context
...
def calculator(expression: str) -> str:
    """Use this tool to evaluate simple mathematical expressions (e.g. '140 * 8' or '1000000000000 * 2').
    Input should be a string containing numbers and arithmetic operators (+, -, *, /)."""
    print(f"\n[Tool Execution: calculator] Evaluating math expression: '{expression}'...")
    try:
        ...
        print(f"[Tool Execution: calculator] Calculated result: {result}")
        return result
    except Exception as e:
        err_msg = f"Error evaluating expression: {e}"
        print(f"[Tool Execution: calculator] {err_msg}")
        return err_msg
...
Thought: {agent_scratchpad}"""
    ...
    tools = [query_video_transcript, calculator]
